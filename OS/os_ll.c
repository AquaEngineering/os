 ...
 *      DEFINES
 *********************/
#define LL_NODE_META_SIZE (sizeof(os_ll_node_t *) + sizeof(os_ll_node_t *))
#define LL_PREV_P_OFFSET(ll_p) (ll_p->n_size)
#define LL_NEXT_P_OFFSET(ll_p) (ll_p->n_size + sizeof(os_ll_node_t *))
 ...
 *  STATIC PROTOTYPES
 **********************/
static void node_set_prev(os_ll_t * ll_p, os_ll_node_t * act, os_ll_node_t * prev);
static void node_set_next(os_ll_t * ll_p, os_ll_node_t * act, os_ll_node_t * next);
...
void * _os_ll_ins_head(os_ll_t * ll_p)
{
    ...
    n_new = os_mem_alloc(ll_p->n_size + LL_NODE_META_SIZE);
...
void * _os_ll_ins_prev(os_ll_t * ll_p, void * n_act)
{
    ...
    if(_os_ll_get_head(ll_p) == n_act) {
        n_new = _os_ll_ins_head(ll_p);
        ...
        os_ll_node_t * n_prev;
        n_prev = _os_ll_get_prev(ll_p, n_act);
...
void _os_ll_remove(os_ll_t * ll_p, void * node_p)
{
    ...
    if(_os_ll_get_head(ll_p) == node_p) {
        /*The new head will be the node after 'n_act'*/
        ll_p->head = _os_ll_get_next(ll_p, node_p);
        if(ll_p->head == NULL) {
            ll_p->tail = NULL;
        }
        else {
    ...
    else if(_os_ll_get_tail(ll_p) == node_p) {
        /*The new tail will be the node before 'n_act'*/
        ll_p->tail = _os_ll_get_prev(ll_p, node_p);
        if(ll_p->tail == NULL) {
            ll_p->head = NULL;
        }
        else {
    	...
    	os_ll_node_t * n_prev = _os_ll_get_prev(ll_p, node_p);
    	os_ll_node_t * n_next = _os_ll_get_next(ll_p, node_p);
...
void _os_ll_clear(os_ll_t * ll_p)
{
    ...
    i      = _os_ll_get_head(ll_p);
    i_next = NULL;

    while(i != NULL) {
        i_next = _os_ll_get_next(ll_p, i);

        _os_ll_remove(ll_p, i);
        os_mem_free(i);
...
void * _os_ll_get_next(const os_ll_t * ll_p, const void * n_act)
{
    ...
    const os_ll_node_t * n_act_d = n_act;
    n_act_d += LL_NEXT_P_OFFSET(ll_p);
    return *((os_ll_node_t **)n_act_d);
}
...
uint32_t _os_ll_get_len(const os_ll_t * ll_p)
{
    ...
    for(node = _os_ll_get_head(ll_p); node != NULL; node = _os_ll_get_next(ll_p, node)) {
        len++;
    }
...
void _os_ll_move_before(os_ll_t * ll_p, void * n_act, void * n_after)
{
    ...
    void * n_before;
    if(n_after != NULL)
        n_before = _os_ll_get_prev(ll_p, n_after);
    else
        n_before = _os_ll_get_tail(ll_p); /*if `n_after` is NULL `n_act` should be the new tail*/

    if(n_act == n_before) return; /*Already before `n_after`*/

    /*It's much easier to remove from the list and add again*/
    _os_ll_remove(ll_p, n_act);
...
static void node_set_prev(os_ll_t * ll_p, os_ll_node_t * act, os_ll_node_t * prev)
{
    ...
    os_ll_node_t ** act_node_p = (os_ll_node_t **) act8;
    os_ll_node_t ** prev_node_p = (os_ll_node_t **) &prev;
...
static void node_set_next(os_ll_t * ll_p, os_ll_node_t * act, os_ll_node_t * next)
{
    ...
    os_ll_node_t ** act_node_p = (os_ll_node_t **) act8;
    os_ll_node_t ** next_node_p = (os_ll_node_t **) &next;
